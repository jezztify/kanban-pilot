...
## Automatic workspace board hosting

Every workspace task set automatically receives an authenticated board endpoint on a random free operating-system port. There is no HTTP port, host, token, or enablement configuration on a workspace. The process-local board credential is newly generated whenever VS Code starts and is never stored in workspace settings.

One Kanban Pilot Registry is automatically shared by every VS Code window in the user profile.
Use **Kanban Pilot: Share** to open its QR code and URL. The Registry lists live workspaces without
board tokens and redirects a selected visitor to the corresponding in-memory board connection.
There is no Registry URL, workspace name, or credential setting; registry startup and enrollment
failures never stop local board serving.
