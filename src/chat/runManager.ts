...
const inFlightReceiptApplications = new Map<string, Promise<void>>();
...
export class RunManager {
	...
	private async applyReceipt(
		...
		if (appliedReceiptKeys.has(key)) {
			return;
		}
		const inFlight = inFlightReceiptApplications.get(key);
		if (inFlight) {
			await inFlight;
			return;
		}

		const application = this.applyReceiptOnce(taskId, runId, stage, receipt, isCurrent, options, key);
		inFlightReceiptApplications.set(key, application);
		try {
			await application;
		} finally {
			if (inFlightReceiptApplications.get(key) === application) {
				inFlightReceiptApplications.delete(key);
			}
		}
	}

	private async applyReceiptOnce(
		taskId: string,
		runId: string,
		stage: Stage,
		receipt: Receipt,
