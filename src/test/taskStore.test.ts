...
suite('task sets', () => {
	...
	test('persists validated independent HTTP ports and tolerates legacy metadata without them', async () => {
		const registry = new TaskSetRegistry(folder, 'legacy/tasks');
		const first = await registry.create('First');
		const second = await registry.create('Second');
		await registry.setHttpPort(first.id, 5101);
		assert.strictEqual((await registry.get(first.id)).httpPort, 5101);
		assert.strictEqual((await registry.get(second.id)).httpPort, undefined);
		await assert.rejects(() => registry.setHttpPort(second.id, 5101), (error: unknown) =>
			error instanceof TaskSetError && error.code === 'duplicate-port',
		);
		await registry.setHttpPort(second.id, 0);
		assert.strictEqual((await registry.get(second.id)).httpPort, 0, 'port zero requests an OS-assigned endpoint port');
		await registry.setHttpPort(first.id, undefined);
		assert.strictEqual((await new TaskSetRegistry(folder, 'legacy/tasks').get(first.id)).httpPort, undefined);
	});
